...
function init() {
  ...
  document.getElementById('btnCloseRecharge').addEventListener('click', () => {
    document.getElementById('rechargeModal').classList.remove('active');
  });
  ...
  document.getElementById('btnCustomRecharge').addEventListener('click', () => {
    const input = document.getElementById('customRecharge');
  ...
  const scanCameraModal = document.getElementById('scanCameraModal');
  ...
  function startQrScanner() {
    ...
    scanCameraModal.classList.add('active');
    ...
    html5QrCode.start(
      ...
      (decodedText) => {
        // 扫码成功，解析二维码内容
        const amount = parseInt(decodedText.trim(), 10);
        if (!isNaN(amount) && amount > 0) {
          // 停止扫码
          stopQrScanner();
          // 显示结果
          scanAmount = amount;
          document.getElementById('scanAmount').textContent = scanAmount;
          scanArea.style.display = 'none';
          scanResult.style.display = 'block';
        } else {
          alert('二维码内容无效，请扫描有效的充值二维码（纯数字）');
        }
      },
      (errorMessage) => {
        // 扫码过程中的错误（通常可以忽略，继续扫描）
        console.log('扫码中...', errorMessage);
      }
    ).catch((err) => {
      console.error('启动摄像头失败:', err);
      alert('无法启动摄像头，请确保已授予摄像头权限，或使用其他充值方式');
      stopQrScanner();
    });
  }
  ...
  function stopQrScanner() {
    ...
    scanCameraModal.classList.remove('active');
  }
  ...
  // 点击摄像头弹窗背景关闭
  scanCameraModal.addEventListener('click', (e) => {
    if (e.target === scanCameraModal) {
      stopQrScanner();
    }
  });
  ...
  document.getElementById('btnConfirmScan').addEventListener('click', () => {
    if (scanAmount > 0) {
      handleRecharge(scanAmount);
      // 重置扫码区域
      scanResult.style.display = 'none';
      scanArea.style.display = 'flex';
      scanArea.querySelector('.scan-placeholder').innerHTML = `
        <span class="scan-icon">📷</span>
        <p>点击打开摄像头扫码</p>
        <p class="scan-hint">扫描充值二维码获取积分</p>
      `;
      scanAmount = 0;
    }
  });
